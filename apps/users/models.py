...
from apps.settings.tasks import send_email_task
...
class User(AbstractUser):
    ...
    country = models.ForeignKey(
        Country, on_delete=models.CASCADE, verbose_name="Страна", blank=True, null=True
    )
    phone_number = models.CharField(
        "Номер телефона", max_length=255, blank=True, null=True
    )
    email = models.EmailField("Почта", max_length=255, blank=True, null=True)

    tarif_usa = models.CharField(
        "Тариф США", max_length=255, blank=True, choices=STATUS_TARIF, default="Новичок"
    )
    tarif_usa_value = models.FloatField("", blank=True, null=True, default=9.5)
    tarif_usa_weight = models.FloatField(
        "Тариф США: Вес", blank=True, null=True, default=0
    )
    ...
    tarif_china = models.CharField(
        "Тариф Китай",
        max_length=255,
        blank=True,
        choices=STATUS_TARIF,
        default="Новичок",
    )
    tarif_china_value = models.FloatField("", blank=True, null=True, default=2.8)
    tarif_china_weight = models.FloatField(
        "Тариф Китай: Вес", blank=True, null=True, default=0
    )
...
class Recipient(models.Model):
    ...
    status_recipient = models.CharField(
        "Статус получателя",
        max_length=255,
        choices=STATUS_RECIPIENT,
        default="Проверяется",
    )
    passport_number = models.CharField(
        "Номер удостоверения личности", max_length=255, blank=True
    )
    passport_place = models.CharField("Кем выдан", max_length=255, blank=True)
    passport_date = models.DateField(
        "Дата выдачи удостоверения личности", blank=True, null=True
    )
    passport_end_date = models.DateField(
        "Дата окончания удостоверения личности", blank=True, null=True
    )
    inn = models.CharField("ИНН", max_length=255, blank=True)
    date_of_birth = models.DateField("Дата рождения", blank=True, null=True)
    passport_image_1 = models.ImageField(
        "Фото удостоверения личности", upload_to="passport_images/", blank=True
    )
    passport_image_2 = models.ImageField(
        "Фото удостоверения личности", upload_to="passport_images/", blank=True
    )
    portret_image = models.ImageField(
        "Портрет", upload_to="portret_images/", blank=True
    )
